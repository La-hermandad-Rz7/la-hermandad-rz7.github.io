...
pairs = [
    ("const canOwnConfig=await rz7Permission('provas.configurar_proprias');","const canOwnConfig=await rz7Permission('provas.configurar_sistema');"),
    ("const canOther=await rz7Permission('provas.configurar_outras');","const canOther=await rz7Permission('provas.configurar_sistema');"),
    ("const canOwnQuestions=await rz7Permission('provas.editar_questoes_proprias');\n const canOtherQuestions=await rz7Permission('provas.editar_questoes_outras');","const canOwnQuestions=await rz7Permission('provas.configurar_sistema');\n const canOtherQuestions=await rz7Permission('provas.configurar_sistema');"),
    ("const canEdit=perms.includes('provas.editar_questoes_outras')||proof.criado_por===profile?.instrutor?.id&&perms.includes('provas.editar_questoes_proprias');","const canEdit=perms.includes('provas.configurar_sistema');"),
    ("Cargo ${escapeHtml(auth.profile.nivel||'instrutor')}","Cargo ${escapeHtml(auth.profile?.cargo?.nome||auth.profile?.nivel||'instrutor')}"),
    ("Cargo atual: <strong>${escapeHtml(auth.profile.nivel||'instrutor')}</strong>","Cargo atual: <strong>${escapeHtml(auth.profile?.cargo?.nome||auth.profile?.nivel||'instrutor')}</strong>")
]
...
function waitForRz7Supabase(timeout=12000){
 ...
 rz7SupabaseWait=new Promise(resolve=>{
